import { Component, Inject, OnInit } from '@angular/core';
 ...
 export class BookingCancellationComponent implements OnInit {
   ...
   loadCancellationPolicy() {
     this.userBookingService.getCancellationPolicy(this.data.bookingId).subscribe({
       next: (policy) => {
         console.log('Policy received from API:', policy); // <-- Add this
         if (policy) {
           this.cancellationPolicy = `
             • Full refund if cancelled ${policy.fullRefundDays}+ days before check-in
             • ${policy.partialRefundDays}% refund if cancelled ${policy.partialRefundDays}-${policy.fullRefundDays} days before check-in
             • No refund if cancelled within ${policy.partialRefundDays} days of check-in
           `;
         } else {
   ...
   calculateRefund() {
     this.userBookingService.calculateRefundAmount(this.data.bookingId).subscribe({
       next: (amount) => {
   ...
   proceedWithCancellation() {
     ...
     this.isLoading = true;
     this.userBookingService.cancelBooking(this.data.bookingId, this.cancellationReason)
       .subscribe({
         next: () => {
